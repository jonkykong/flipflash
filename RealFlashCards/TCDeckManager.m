...
+ (TCDeckManager *)sharedManager{
    ...
    dispatch_once(&pred, ^{
        ...
        if(decksData) {
            sharedManager.editDecks = [NSKeyedUnarchiver unarchiveObjectWithData:decksData];
        } else {
            ...
            for(NSUInteger i = 0; i < 8; i++) {
                ...
                switch (i) {
                    case 0:
                        card.frontText = @"Swipe right when you know a card →";
                        break;
                    case 1:
                        card.frontText = @"← Swipe left when you don't";
                        break;
                    case 2:
                        card.frontText = @"Tap a card to flip it over";
                        card.backText = @"That's it!";
                        break;
                    case 3:
                        card.frontText = @"Hold your phone sideways to make the cards larger...";
                        break;
                    case 4:
                        card.frontText = @"...or pinch to zoom in even more!";
                        break;
                    case 5:
                        card.frontText = @"Add photos to cards from your camera";
                        card.frontImage = [UIImage imageNamed:@"math.jpg"];
                        break;
                    case 6:
                        card.frontText = @"Send decks you create to your friends";
                        break;
                    case 7:
                        card.frontText = @"Tap ☰ in the top-right corner to see more options";
                        break;
                }
                [tutorialDeck.cards addObject:card];
            }
            
            sharedManager.editDecks = [NSMutableArray arrayWithObject:tutorialDeck];
...
- (void)addDeck:(TCDeck *)deck {
    ...
    if(![editDecks containsObject:deck]) {
        [editDecks addObject:deck];
        NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"title" ascending:YES];
        [editDecks sortUsingDescriptors:@[ sortDescriptor ]];
    }
    [self saveDecks];
}

- (void)removeDeck:(TCDeck *)deck {
    [editDecks removeObject:deck];
}

- (void)saveDecks {
    [saveToDiskQueue addOperationWithBlock:^{
        NSMutableArray *decksToSave = [NSMutableArray arrayWithArray:editDecks];
